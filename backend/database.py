...
db_url = f"mysql+pymysql://{DBUSER}:{DBPASSWORD}@{DBHOST}:{DBPORT}/{DBNAME}"
...
# Create tables if they don't exist and populate states if empty
...
# Insert states if table empty
check_states = text("SELECT COUNT(*) as cnt FROM states;")
with engine.connect() as conn:
    res = conn.execute(check_states).mappings().all()
    ...
    if cnt == 0:
        ...
        insert_stmt = text(
            "INSERT INTO states (name, capital, region, slogan, population, landmarks) VALUES (:name, :capital, :region, :slogan, :population, :landmarks)"
        )
